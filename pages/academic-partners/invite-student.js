...
const InviteStudentByFaculty = ({ usaStates }) => {
  const currentUser = useSelector(getCurrentUser);
  const usaState =
    currentUser && currentUser?.usa_state && currentUser?.usa_state?.id;
  const university =
    currentUser && currentUser?.university && currentUser?.university?.id;

...
const ProgramsContent = ({ workBookContent, userId }) => {
  ...
  const interactiveLessonIds = [
    1.1, 2.4, 3.8, 4.5, 5.5, 6.9, 7.6, 8.2, 9.7, 10.8, 12.1, 13.5, 13.6, 11.4,
    14.1, 14.2,
  ];
  ...
  useEffect(() => {
    ...
    if (lessonIndex === 71) setPrevLessonId(false);
  ...
  const onClickNextLesson = async (lastLesson) => {
    ...
    (async () => {
      try {
        ...
        const workBookReport = userId && (await storeWorkbookResponse(dataObj));
      } catch (err) {
        console.log("Error occured while updating workbook");
      }
    })();
      ...
      <div className="w-full">
        <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
          <div className="bg-[#F8FCFF] min-h-[100vh]">
            <div className="p-5">
              <h3 className="text-[#363F48] text-[17px] font-bold">Workbook</h3>
            ...
            <div className="table_contents_scroll mt-[11px]">
              {workBookContent &&
                workBookContent?.data &&
                workBookContent.data.map((item, key) => {
                  return (
                    <TableContent
                      item={item}
                      key={key}

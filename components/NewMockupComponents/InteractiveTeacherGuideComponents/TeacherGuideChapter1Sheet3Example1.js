...
import { TeacherGuideChapter1Sheet3Example1Data } from "./data";
...
const TeacherGuideChapter1Sheet2Example1 = ({
  ...
  useEffect(() => {
    let chapter1sheet3example1Report = null;
    if (workBookAPI && workBookAPI?.chapter1sheet3example1) {
      chapter1sheet3example1Report = JSON.parse(workBookAPI?.chapter1sheet3example1);
      setInputValues(chapter1sheet3example1Report);
    }
  }, [workBookAPI]);

  const onClickFormSubmission = async () => {
    if (inputValues.c1s3e1q1 == "") {
      return setErrors("c1s3e1q1");
    }
  ...
  return (
    <div className="w-full">
      ...
      <form onSubmit={view !== "faculty" && onClickFormSubmission}>
        {TeacherGuideChapter1Sheet3Example1Data.map((interactiveItem, ind) => {
          return (
            <TextFieldComponent

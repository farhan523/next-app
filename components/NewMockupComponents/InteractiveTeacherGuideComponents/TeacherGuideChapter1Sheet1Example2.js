...
import { TeacherGuideChapter1Sheet1Example2Data } from "./data";
import NextPrevLesson from "../Dashboard/ProgramsContent/NextPrevLesson";
const TeacherGuideChapter1Sheet1Example2 = ({
  handleSubmitForm,
  workBookAPI,
  onClickNextLesson,
  onClickPrevLesson,
  ...
  useEffect(() => {
    let chapter1sheet1example2Report = null;
    if (workBookAPI && workBookAPI?.chapter1sheet1example2) {
        chapter1sheet1example2Report = JSON.parse(workBookAPI?.chapter1sheet1example2);
      setInputValues(chapter1sheet1example2Report);
    }
  }, [workBookAPI]);

  const onClickFormSubmission = async () => {
    if (inputValues.c1s1e2q1 == "") {
      return setErrors("c1s1e2q1");
    }
    if (inputValues.c1s1e2q2 == "") {
      return setErrors("c1s1e2q2");
    }
    if (inputValues.c1s1e2q3 == "") {
        return setErrors("c1s1e2q3");
      }
  ...
  return (
    <div className="w-full">
      ...
      <form onSubmit={view !== "faculty" && onClickFormSubmission}>
        {TeacherGuideChapter1Sheet1Example2Data.map((interactiveItem, ind) => {
          return (
            <TextFieldComponent
...
export default TeacherGuideChapter1Sheet1Example2;

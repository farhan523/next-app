...
import { TeacherGuideChapter5Sheet1Data } from "./data";
import NextPrevLesson from "../Dashboard/ProgramsContent/NextPrevLesson";
const TeacherGuideChapter5Sheet1 = ({
  handleSubmitForm,
  workBookAPI,
  onClickNextLesson,
  onClickPrevLesson,
  ...
  useEffect(() => {
    let chapter5sheet1Report = null;
    if (workBookAPI && workBookAPI?.chapter5sheet1) {
      chapter5sheet1Report = JSON.parse(workBookAPI?.chapter5sheet1);
      setInputValues(chapter5sheet1Report);
    }
  }, [workBookAPI]);

  const onClickFormSubmission = async () => {
    if (inputValues.c5s1q1 == "") {
      return setErrors("c5s1q1");
    }
    if (inputValues.c5s1q3 == "") {
        return setErrors("c5s1q3");
      }
  ...
  return (
    <div className="w-full">
      ...
      <form onSubmit={view !== "faculty" && onClickFormSubmission}>
        {TeacherGuideChapter5Sheet1Data.map((interactiveItem, ind) => {
          return (
            <TextFieldComponent
...
export default TeacherGuideChapter5Sheet1;

...
import { TeacherGuideChapter1Sheet2Example2Data } from "./data";
...
const TeacherGuideChapter1Sheet2Example1 = ({
  ...
  useEffect(() => {
    let chapter1sheet2example2Report = null;
    if (workBookAPI && workBookAPI?.chapter1sheet2example2) {
      chapter1sheet2example2Report = JSON.parse(workBookAPI?.chapter1sheet2example2);
      setInputValues(chapter1sheet2example2Report);
    }
  }, [workBookAPI]);
  ...
  return (
    <div className="w-full">
      ...
      <form onSubmit={view !== "faculty" && onClickFormSubmission}>
        {TeacherGuideChapter1Sheet2Example2Data.map((interactiveItem, ind) => {
          return (
            <TextFieldComponent

...
export async function createLoginAudit(body) {
  try {
    ...
    // console.log(LoginAudit.data.data.id,"LoginAudit")
    localStorage.setItem("logid",LoginAudit.data.data.id);
    console.log("logcreated",LoginAudit);
    const logs = LoginAudit.data.data.attributes.activityLogs

  

      localStorage.setItem("previousdatabaselogs",logs)
...
export async function updateLoginAudit(logId,logs,leaveTime) {
    try{
      const updatedData = {
        data:{
          activityLogs:logs,
      ...
      const response = await fetch(
        `${NEXT_PUBLIC_STRAPI_URL}/api/login-audits/${logId}`,
        {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
      ...
      console.log("updatedUserResponse",updateUserResponse);
      const logsUpdated = updateUserResponse.data.attributes.activityLogs;
      console.log("bigo",updateUserResponse.data.attributes.activityLogs); 
      
      localStorage.setItem("previousdatabaselogs",logsUpdated)
      return updateUserResponse;
    }catch(e){
      return { error: true, message: e?.message, errorObj: e?.response?.data?.error };
    }
...
export async function getLoginAuditById(logId){
    try{
        const LoginAudit = await axios.get(`${NEXT_PUBLIC_STRAPI_URL}/api/login-audits/${logId}`)
        console.log("loginAudit",LoginAudit)
        return LoginAudit;
    }catch(e){
      return { error: true, message: e?.message, errorObj: e?.response?.data?.error };
    }
